...
为避免在额外生成的代码上的开销 和 额外的非直接的对枚举成员的访问，我们可以使用 const 枚举，即常量枚举。
...
常量枚举成员在使用的地方会被内联进来； 因为**常量枚举不允许包含计算成员**。
...
外部枚举 和 非外部枚举有一个重要的区别：

```
      对于正常的枚举，没有初始化方法的成员被当做常数成员；

      对于非常数的外部枚举，没有初始化方法时被当做需要经过计算的；
```
